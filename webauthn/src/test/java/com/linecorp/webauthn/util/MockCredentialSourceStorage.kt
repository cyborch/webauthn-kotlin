...
class MockCredentialSourceStorage : CredentialSourceStorage {
    ...
    override fun store(credSource: com.linecorp.webauthn.model.PublicKeyCredentialSource) {
    ...
    override fun load(credId: String): com.linecorp.webauthn.model.PublicKeyCredentialSource? {
        val credSourceEntity = credSourceTable.firstOrNull { it.credId.contentEquals(credId) }
        return credSourceEntity?.let {
            com.linecorp.webauthn.model.PublicKeyCredentialSource(
                type = it.credType,
                id = it.credId,
                rpId = it.rpId,
    ...
    override fun loadAll(aaguid: UUID?): List<com.linecorp.webauthn.model.PublicKeyCredentialSource> = credSourceTable
        .filter { entity ->
